...
# pulls the image, hashes an admin password, writes a .env, starts Postgres +
...
PASSWORD_HASH="$($DOCKER run --rm "$IMAGE" bun dist/index.js hash "$ADMIN_PASSWORD")" \
  || die "Failed to hash the admin password"
[ -n "$PASSWORD_HASH" ] || die "Password hashing produced no output"
...
info "  • Rate-limit /api/auth/ at that proxy — the login path runs scrypt and is"
info "    the one password an attacker can guess. See the README for examples."
printf '\n'
